...
// get tasks that have submitted a age filter
elseif(array_key_exists("age",$_GET)) {
  
  // get age from query string
  $age = $_GET['age'];

  // check to see if age in query string is either Y or N
  if(!is_numeric($age)) {
    $response = new Response();
    $response->setHttpStatusCode(400);
    $response->setSuccess(false);
    $response->addMessage("make sure you enter numeric value");
    $response->send();
    exit;
  }
  ...
  if($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
      // create db query
      $query = $readDB->prepare('SELECT id, name, fname, DATE_FORMAT(dob, "%d/%m/%Y %H:%i") as dob, age from tbltasks where age like :age');
